    ...
    tabItem(tabName = "modeling",
            tabsetPanel(
              tabPanel("Modeling Info",
                       ...
                       h4("It is a machine learning algorithm based on supervised learning. This method model the relationship between a dependent variable and multiple explanatory (independent) variables. It also predict the outcome of the response variable. The expression of the regression function is given as: "),
                       h4(withMathJax(helpText("$$Y_i=\\beta_0+\\beta_1x_{i1}+\\beta_2x_{i2}+\\beta_px_{ip}+...+E$$"))),
                       br(),
                       h4("Benefits: It allow easy prediction of response and inferences. The understanding and interpretation of each variable can be given according to the coefficient.  The modeling speed is fast, does not require very complicated calculations."),
                       h4("Drawbacks: It requires assumptions check, such as constant variance/residuals are normally distributed/no multicollinearity between predictors/linear relationship between the response variable and the predictors. It is also sensitive to outliers. 
"),
                       h3("Regression Tree Model"),
                       h4("It is a tree based method which split up predictor space into regions, and make different predictions for each region. The goal is to predict a continuous response, and use mean of observations as prediction. The split is picked by using recursive binary splitting and find the minimized RSS for every possible value of each predictor. Once the first split is chosen, the same process is used to create the second split and thus grow to a large tree with many nodes. The expression of the RSS is given as:"),
                       h4(withMathJax(helpText("$$R_1(j,s) = (x|x_j <s)$$"))),
                       h4(withMathJax(helpText("$$R_2(j,s) = (x|x_j \\ge s)$$"))),
                       h4("Benefits: It is simple to understand and interpret output, predictors don’t need to be scaled. No statistical assumptions necessary, and it has built in variable selection."),
                       h4("Drawbacks: A small changes in data can vastly change tree, greedy algorithm necessary and it usually need to prune. It lose interpretability."),
                       br(),
                       h3("Random Forest Model"),
                       h4("It use same idea as bagging, which to create multiple trees from bootstrap samples and average the predictions. Instead of using all predictors, It use a random subset of predictors for each bootstrap sample/tree fit. The expression of randonly selected predictors is given as: "),
                       h4(withMathJax(helpText("$$m=\\frac{p}{3}$$"))),
                       h4("Benefits: It can come out with very high dimensional (features) data, and no need to reduce dimension, no need to make feature selection. Since average across trees, improve prediction."),
                       h4("Drawbacks: It lose interpretability and need to choose the number of trees. ")),
